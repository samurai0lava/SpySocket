...
void Servers::epollFds(Servers &serv)
{
	...
	while (true)
	{
		...
		for (int i = 0; i < ready_fds; ++i)
		{
			...
			if (it != serv.serversFd.end())
			{
				...
				if (epoll_ctl(epollFd, EPOLL_CTL_ADD, client_fd, &client_ev) ==
					-1)
				{
			...
			if (events[i].events & EPOLLIN)
			{
				...
				ParsingRequest::ParseResult result = parser->feed_data(serv.buffer,
						serv.bufferLength);
			...
			else if (events[i].events & EPOLLOUT)
			{
				...
				{
					if (c.response.empty()
						&& client_data_map[fd].chunkedSending == false)
					{
					...
					ssize_t bytes_sent = send(fd, c.response.c_str(),
							c.response.size(), MSG_NOSIGNAL);
					if (bytes_sent == -1)
					{
					...
					if (c.response.empty())
					{
						...
						if (client_data_map[fd].chunkedSending == true)
						{
							...
							memset(&ev, 0, sizeof(ev));
							ev.events = EPOLLIN;
								// Reset to listen for new requests
							ev.data.fd = fd;
	...
	for (std::map<int,
		ParsingRequest *>::iterator it = clientParsers.begin(); it != clientParsers.end(); ++it)
	{
		delete it->second;
	}
	clientParsers.clear();
